...
let checkboxCats = document.querySelectorAll('.categories-search__checkbox');

for (let index = 0; index < checkboxCats.length; index++) {
	const checkboxCat = checkboxCats[index];
	checkboxCat.addEventListener("change", function (e) {
		checkboxCat.classList.toggle('_active');

		let checkboxActiveCats = document.querySelectorAll('.categories-search__checkbox._active');

		if (checkboxActiveCats.length > 0) {
			searchSelect.classList.add('_categories');
			let searchQuantity = searchSelect.querySelector('.search-page__quantity');
			searchQuantity.innerHTML = searchQuantity.getAttribute('data-text') + ' ' + checkboxActiveCats.length;
		} else {
